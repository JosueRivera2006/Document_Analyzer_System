...
import tempfile
# librerias para la exportacion de pdf
...
# Exportar el resumen a docx
@app.post("/resumir/exportar/docx", summary="Resumir y descargar como DOCX")
async def resumir_y_exportar_docx(archivo: UploadFile = File(...)):
    nombre = archivo.filename or "documento"
    extension = nombre.lower().split('.')[-1]
    ...
    ruta_docx = tempfile.NamedTemporaryFile(delete=False, suffix=".docx").name

    doc = Document()
    doc.add_heading(resultado["titulo"], 0)
    doc.add_paragraph(resultado["resumen"])
    doc.save(ruta_docx)

    return FileResponse(
        path=ruta_docx,
        filename=f"resumen_{os.path.splitext(nombre)[0]}.docx",
        media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    )

# Exportar el resumen a pptx
@app.post("/resumir/exportar/pptx", summary="Resumir y descargar como PPTX")
async def resumir_y_exportar_pptx(archivo: UploadFile = File(...)):
    nombre = archivo.filename or "documento"
    extension = nombre.lower().split('.')[-1]
    ...
    ruta_pptx = tempfile.NamedTemporaryFile(delete=False, suffix=".pptx").name

    prs = Presentation()

    # Slide de título
    slide = prs.slides.add_slide(prs.slide_layouts[0])
    slide.shapes.title.text = resultado["titulo"]

    # Dividir resumen por párrafos y crear una slide por párrafo
    for parrafo in resultado["resumen"].split("\n"):
        if parrafo.strip():  # evitar párrafos vacíos
            slide = prs.slides.add_slide(prs.slide_layouts[1])
            slide.shapes.title.text = "Resumen"
            slide.placeholders[1].text = parrafo.strip()

    prs.save(ruta_pptx)

    return FileResponse(
        path=ruta_pptx,
        filename=f"resumen_{os.path.splitext(nombre)[0]}.pptx",
        media_type="application/vnd.openxmlformats-officedocument.presentationml.presentation"
    )

# Exportar el resumen a pdf
@app.post("/resumir/exportar/pdf", summary="Resumir y descargar como PDF")
async def resumir_y_exportar_pdf(archivo: UploadFile = File(...)):
    nombre = archivo.filename or "documento"
    extension = nombre.lower().split('.')[-1]
    ...
    ruta_pdf = tempfile.NamedTemporaryFile(delete=False, suffix=".pdf").name

    doc = SimpleDocTemplate(ruta_pdf)
    styles = getSampleStyleSheet()

    contenido = []
    contenido.append(Paragraph(resultado["titulo"], styles["Title"]))
    contenido.append(Spacer(1, 12))
    contenido.append(Paragraph(resultado["resumen"], styles["BodyText"]))

    doc.build(contenido)

    return FileResponse(
        path=ruta_pdf,
        filename=f"resumen_{os.path.splitext(nombre)[0]}.pdf",
